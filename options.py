...
class OptionParser:
    def __init__(self):
        ...
        self.argp_checking = self.argp.add_argument_group('checking arguments')
        self.argp_checking.add_argument('--parse-only', action = 'store_true', help = 'only parse the input file')
        self.argp_checking.add_argument('--max-threads', type = int, metavar = 'n', default = '-1', help = 'number of threads to use; #processors+n if n<=0')
        self.argp_checking.add_argument('--timeout', type = int, metavar = 'seconds', default = 0, help = 'timeout for individual checks')

        self.argp_output = self.argp.add_argument_group('output arguments')
        self.argp_output.add_argument('--outputfile', metavar = 'filename', default = 'delta.out.smt2', help = 'filename for the output file')
        self.argp_output.add_argument('--pretty-print', action = 'store_true', help = 'pretty-print to output file')
        self.argp_output.add_argument('--wrap-lines', action = 'store_true', help = 'wrap lines in output file')
